...
def download_single_watershed_data(watershed, ecmwf_rapid_prediction_location):
    """
    Loads single watersheds datasets from data store
    """
    #get data engine
    ...
    if 'ckan' == data_store.data_store_type.code_name:
        data_manager = ERFPDatasetManager(data_store.api_endpoint,
                               data_store.api_key,
                               ecmwf_rapid_prediction_location)                      
...
def load_datasets():
    """
    Loads prediction datasets from data store for all watersheds
    """
    session = SettingsSessionMaker()
    main_settings  = session.query(MainSettings).order_by(MainSettings.id).first()
    ecmwf_rapid_prediction_location = main_settings.local_prediction_files
    if ecmwf_rapid_prediction_location and os.path.exists(ecmwf_rapid_prediction_location):
        for watershed in session.query(Watershed).all():
            download_single_watershed_data(watershed, ecmwf_rapid_prediction_location)
    else:
        print "Prediction location invalid. Please set to continue"
